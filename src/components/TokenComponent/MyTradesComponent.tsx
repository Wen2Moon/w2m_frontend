...
import { GET_MY_TRADES } from "@/apollo/queries/tradesQuery";
...
const MyTradesComponent: React.FC<RecentTradesComponentProps> = ({
  tokenSymbolIn = "W2M",
  tokenSymbolOut = "PEPE",
  itemsPerPage = 20,
  poolAddress,
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const { data } = await client.query({
        query: GET_MY_TRADES,
        variables: {
          first: 100, // Giới hạn số lượng để tránh overload
          orderDirection: "desc",
          orderBy: "timestamp",
          account: address,
          pool: poolAddress,
        ...
        <tbody>
          {currentData.map((trade, index) => (
            ...
            >
              ...
              <td className="py-3 px-6 max-w-[60px]">{trade.amountIn}</td>
